		...
		<table class="table table-hover">
			<tbody>
				...
				<tr>
					...
					<td>: <?=($r['tgl_kembali']) ? date("d-m-Y H:i:s", strtotime($r['tgl_kembali'])) : "<b>Belum</b>"?>
					</td>
				</tr>
				<tr>
					<th>Jatuh Tempo</th>
				...
				<tr>
					<th>Konfirmasi</th>
					<td>: <span
							class="label label-<?=($r['konfirmasi']) ? "success" : "danger"?>"><?=($r['konfirmasi']) ? "Sudah" : "Belum"?></span>
					</td>
				</tr>
				<tr>
					<th>Kembali</th>
					<td>: <span
							class="label label-<?=($r['status']) ? "success" : "danger"?>"><?=($r['status']) ? "Sudah" : "Belum"?></span>
					</td>
				</tr>
				<tr>
					<th>Pembatalan</th>
					<td>: <span
							class="label label-<?=($r['pembatalan']) ? "danger" : "success"?>"><?=($r['pembatalan']) ? "Ya" : "Tidak"?></span>
					</td>
				</tr>
				<tr class="hidden-print">
					...
					<td> &nbsp;
						<?php if (!$r['konfirmasi']): ?>
						<a href="?page=konfirmasi&id=<?= $r['id_transaksi'] ?>" class="btn btn-primary">Konfirmasi
							Sekarang</a>
						<?php endif ?>
						<?php if ($r['konfirmasi'] == 1 AND $r['tgl_kembali'] == NULL AND $r["pembatalan"] != 1): ?>
						<a href="?page=perpanjang&id=<?= $r['id_transaksi'] ?>" class="btn btn-success">Perpanjang
							Sekarang</a>
						<?php endif ?>
					</td>
				</tr>
